...
function displayLocationWeather(location) {
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${location.latitude}&longitude=${location.longitude}&current=temperature_2m,is_day,precipitation,rain,showers,snowfall,cloud_cover,wind_speed_10m&hourly=temperature_2m,precipitation_probability,precipitation,rain,showers,snowfall,cloud_cover,cloud_cover_low,cloud_cover_mid,cloud_cover_high&temperature_unit=fahrenheit&precipitation_unit=inch&timezone=auto`).then((response)=>response.json()).then((locationWeatherData)=>{
        ...
        const current_time_formatted = locationWeatherData.current.time.slice([
            11
        ]);
...
function createHourlyForecastTable(weatherData, location) {
    ...
    for(let i = 0; i < weatherData.length; i++){
        ...
        tdTagHour.textContent = weatherData[i].time.slice([
            11
        ]);
...
function currentWeatherCondition(weatherData) {
    ...
    function dayOrNight() {
        if (weatherData.current.is_day === 1) return "It's daytime!";
        else return "It's nighttime!";
    }
    function clouds() {
        if (weatherData.current.cloud_cover <= 30) return "Today the sky is clear!";
        else if (weatherData.current.cloud_cover <= 60) return "There are a few clouds in the sky...";
        else if (weatherData.current.cloud_cover <= 90) return "The sky is mostly cloudy right now...";
        else return "Completely overcast";
    }
    function isRaining() {
        if (weatherData.current.rain === 0) return "No rain";
        else if (weatherData.current.rain <= 0.5) return "Light rain";
        else if (weatherData.current.rain <= 4.0) return "Moderate rain";
        else return "Heavy Rain";
    }
    function isSnowing() {
        if (weatherData.current.snowfall === 0) return "It is not snowing";
        else if (weatherData.current.snowfall <= 0.5) return "Light snow";
        else if (weatherData.current.snowfall <= 4.0) return "Moderate snow";
